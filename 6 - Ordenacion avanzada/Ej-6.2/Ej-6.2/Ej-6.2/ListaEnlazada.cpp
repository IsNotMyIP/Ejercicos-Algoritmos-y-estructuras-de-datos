...
void ListaEnlazada::ordenarPorMergeSort(ListaEnlazada &lista) {
	...
	if (lista.getN() >= 2) {
		...
		// En ese momento ponemos la lista no vacía tal cual a continuación
		...
		// No podemos usar el operador de concatenar porque entonces estaríamos teniendo memoria adicional O(n)
		while (sublistaNoVacia->getN() > 0) {
			lista.insertar(lista.getN(), sublistaNoVacia->getValor(0));
			sublistaNoVacia->eliminar(0); // Tenemos que ir eliminando a medida que vamos añadiendo en el otro lado, para que la memoria adicional no sea O(n)
		}
